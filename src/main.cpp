...
void setup()
{
    ...
    esp_sleep_enable_ext0_wakeup(PIN_POWER_SWITCH, HIGH); // setup wake from sleep
    ...
    attachInterrupt(PIN_POWER_SWITCH, deep_sleep_start_isr, LOW); // attach interrupt to start sleep immediately on power toggle during setup
    ...
    // if (SPIFFS.exists("/index-setup.html")) {
    //     print("File exists\n");
    // } else {
    ...
    // while (true)
    // {
    //     Serial.println("Button 1: " + String(digitalRead(PIN_BUTTON_MODE)));
    //     Serial.println("Button 2: " + String(digitalRead(PIN_BUTTON2_MODE)));
    //     delay(250);
    // }

    // CLI setup
    if (digitalRead(PIN_BUTTON_MODE) == LOW && digitalRead(PIN_BUTTON2_MODE) == HIGH) {
        Serial.print("CLI mode\n");
...
void task_spotify_code(void* parameter)
{
    ...
    if (!prefs.getString(PREFS_SPOTIFY_CLIENT_ID_KEY, client_id, CLI_MAX_CHARS) || !prefs.getString(PREFS_SPOTIFY_AUTH_B64_KEY, auth_b64, CLI_MAX_CHARS) || !prefs.getString(PREFS_SPOTIFY_REFRESH_TOKEN_KEY, refresh_token, CLI_MAX_CHARS)) {
